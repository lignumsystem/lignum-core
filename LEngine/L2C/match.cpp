			    ...
			    CallerData& caller_data)
{
  ...
  //"consider only Modules in consider set and brackets"
  ...
  if (pos.GetModuleId() == head){
    ...
    if (MatchLeftContext(pos,pp,caller_data,ignore_set,consider_set)){
      cout << "is a left context. " << endl;
    }
    else
      //if false already here return
      return false;
    if (MatchRightContext(pos,pp,caller_data,ignore_set,consider_set)){
      cout << "is a right context. " << endl;
      return true;
    }
    else
      return false;
    ...
    //not even strict predecessor match
    return false;
}
