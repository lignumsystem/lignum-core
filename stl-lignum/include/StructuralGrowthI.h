  ...
  (GCSegment* ms, TreeCompartment<GCSegment, GCBud>* tc)const
{
  if(Axis<GCSegment,GCBud>* ax = dynamic_cast<Axis<GCSegment,GCBud>*>(tc)) {
    ...
    if(GetValue(*bud,LGAage) > 0.0 && (GetValue(*bud,state) == ALIVE)) {
      ...
      LGMdouble l_new = GetValue(tree, LGPlr) * r_new;
      LGMdouble go = GetValue(*bud, LGAomega);
      LGMdouble rh_new = sqrt(GetValue(tree,LGPxi))*r_new;
	...
	to_side = Cross(to_stemp,up);
      ...
      BroadLeaf* leaf =
	new BroadLeaf(GetValue(tree,LGPsf),0.06,GetValue(tree,dof_p),no_sect,
	...
	(pow(r_new,2.0)-pow(rh_new,2.0))/GetValue(*leaf,LGAdof);
      ...
      InsertLeaf(*ts, leaf);
      SetValue(*ts,LGAWf,GetValue(tree,dof_p)*ellipsis_a/GetValue(*leaf,LGAsf));
      SetValue(*ts,LGAWs,GetValue(tree,LGPrhoW)*PI_VALUE*
	       (pow(r_new,2.0)-pow(rh_new,2.0))*l_new);
      SetValue(*ts,Wh,GetValue(tree,LGPrhoW)*PI_VALUE*pow(rh_new,2.0)*l_new);
      SetValue(*ts,dAs,0.0);
      SetValue(*ts,dW,0.0);
      SetValue(*ts,LGAdR,0.0);
