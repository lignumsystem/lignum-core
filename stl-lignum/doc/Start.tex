...
\caption{\label{fig:segment} Tree segment for a coniferous tree (left).
Tree segment for a deciduous tree (right). H = heartwood, S =
...
photosynthetically  active radiation  (PAR).  Many  algorithms  can be
developed   to  compute   the  PAR   but  in   \lignum\   the  current
implementation   is  based   on  the   pairwise  comparison   of  tree
segments.  The complexity  of the  current algorithm  is  $O(n^2)$.  A
better way could be to first create a (linear) algorithm using, e.g.,
Accumulate that collects  the tree segments to a  new data structure.
Then with the help of some heuristics a better than $O(n^2)$ algorithm
for the computation of PAR might be found.
